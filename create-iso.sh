#!/bin/bash
#
# OrchanixOS LiveCD ISO Creator - Copyright (C) 2022 OrchanixOS Developers.
#
...
which orchanixos-chroot &>/dev/null || (echo "Error: orchanixos-chroot (part of OrchanixOS) is required." >&2; exit 1)
...
# Create directories.
mkdir -p iso-workdir/{iso-root,orchanixos-rootfs,mnt,squashfs-tmp,syslinux}
...
tar -xpf "$1" -C iso-workdir/orchanixos-rootfs
ver="$(cat iso-workdir/orchanixos-rootfs/etc/orchanixos-release)"
# Prepare the live system.
echo "Preparing the live system..."
chroot iso-workdir/orchanixos-rootfs /usr/sbin/groupadd -r autologin
chroot iso-workdir/orchanixos-rootfs /usr/sbin/useradd -c "Live User" -G wheel,netdev,lpadmin,autologin -ms /bin/bash orchanixos
echo "orchanixos:orchanixos" | chroot iso-workdir/orchanixos-rootfs /usr/sbin/chpasswd -c SHA512
echo "orchanixos ALL=(ALL) NOPASSWD: ALL" > iso-workdir/orchanixos-rootfs/etc/sudoers.d/live
cat > iso-workdir/orchanixos-rootfs/etc/polkit-1/rules.d/49-live.rules << "END"
polkit.addRule(function(action, subject) {
    if (subject.isInGroup("orchanixos")) {
        return polkit.Result.YES;
    }
});
END
install -m755 livecd-installer iso-workdir/orchanixos-rootfs/usr/bin/livecd-installer
install -m644 livecd-installer.desktop iso-workdir/orchanixos-rootfs/usr/share/applications/livecd-installer.desktop
chroot iso-workdir/orchanixos-rootfs /usr/bin/install -o orchanixos -g orchanixos -dm755 /home/orchanixos/Desktop
chroot iso-workdir/orchanixos-rootfs /usr/bin/install -o orchanixos -g orchanixos -m755 /usr/share/applications/livecd-installer.desktop /home/orchanixos/Desktop/livecd-installer.desktop
install -m644 livecd-files/trust-livecd-installer.desktop iso-workdir/orchanixos-rootfs/etc/xdg/autostart/trust-livecd-installer.desktop
...
install -d iso-workdir/orchanixos-rootfs/usr/lib/firmware
pushd iso-workdir/firmware >/dev/null
patch -sNp1 -i ../../livecd-files/linux-firmware-compression.patch
./copy-firmware.sh -C "$PWD"/../orchanixos-rootfs/usr/lib/firmware
install -t "$PWD"/../orchanixos-rootfs/usr/lib/firmware -Dm644 GPL-2 GPL-3 LICENCE* LICENSE* WHENCE
popd >/dev/null
install -d iso-workdir/orchanixos-rootfs/usr/lib/firmware/intel-ucode
install -m644 iso-workdir/mcode/intel-ucode{,-with-caveats}/* iso-workdir/orchanixos-rootfs/usr/lib/firmware/intel-ucode
pushd iso-workdir/sof >/dev/null
cp -r sof*$SOF_VER "$PWD"/../orchanixos-rootfs/usr/lib/firmware/intel
ln -sf sof-$SOF_VER "$PWD"/../orchanixos-rootfs/usr/lib/firmware/intel/sof
ln -sf sof-tplg-$SOF_VER "$PWD"/../orchanixos-rootfs/usr/lib/firmware/intel/sof-tplg
install -t "$PWD"/../orchanixos-rootfs/usr/lib/firmware/intel/sof -Dm644 LICENCE.Intel LICENCE.NXP Notice.NXP
popd >/dev/null
# Create Squashfs image.
echo "Creating squashfs image..."
cd iso-workdir/orchanixos-rootfs
mksquashfs * ../iso-root/LiveOS/squashfs.img -comp xz -quiet
cd ../..
# Copy kernel and generate initramfs.
echo "Copying kernel..."
cp iso-workdir/orchanixos-rootfs/boot/vmlinuz* iso-workdir/iso-root/vmlinuz
echo "Generating initramfs..."
orchanixos-chroot iso-workdir/orchanixos-rootfs /usr/bin/dracut -q -a dmsquash-live initramfs.img "$(ls iso-workdir/orchanixos-rootfs/usr/lib/modules)" >/dev/null
echo "Copying initramfs..."
mv iso-workdir/orchanixos-rootfs/initramfs.img iso-workdir/iso-root/initramfs.img
...
# EFI.
mkdir -p iso-workdir/orchanixos-rootfs/boot/grub
cp livecd-files/grub.cfg iso-workdir/orchanixos-rootfs/boot/grub/grub.cfg
orchanixos-chroot iso-workdir/orchanixos-rootfs /usr/bin/grub-mkstandalone -d /usr/lib/grub/x86_64-efi -O x86_64-efi -o BOOTX64.EFI --compress=xz /boot/grub/grub.cfg >/dev/null
cp iso-workdir/orchanixos-rootfs/BOOTX64.EFI iso-workdir/iso-root/EFI/BOOT/BOOTX64.EFI
chmod +x iso-workdir/iso-root/EFI/BOOT/BOOTX64.EFI
cp iso-workdir/orchanixos-rootfs/usr/share/grub/unicode.pf2 iso-workdir/iso-root/unicode.pf2
cp iso-workdir/orchanixos-rootfs/usr/share/licenses/grub/COPYING iso-workdir/iso-root/EFI/BOOT/LICENSE-BOOTX64.txt
...
mkfs.fat -F12 iso-workdir/iso-root/EFI/BOOT/efiboot.img -n "orchanixos_EFI"
...
xorrisofs -iso-level 3 -d -J -N -R -max-iso9660-filenames -relaxed-filenames -allow-lowercase -V "ORCHANIXOS" -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e EFI/BOOT/efiboot.img -isohybrid-gpt-basdat -no-emul-boot -isohybrid-mbr iso-workdir/iso-root/isolinux/isohdpfx.bin -o "orchanixos-$ver-x86_64-$variant.iso" iso-workdir/iso-root
# Clean up.
echo "Cleaning up..."
rm -rf iso-workdir
# Finishing message.
echo "All done! Output image written to orchanixos-$ver-x86_64-$variant.iso."
